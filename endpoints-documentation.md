...
### im.sendAction
Отправляет системное действие / экшн (сервисное сообщение) в указанную беседу или диалог от лица отправителя.

Параметры:
* `peer_id` (int64) — ID получателя/беседы (или `user_id` / `chat_id`).
* `action_type` (string) — тип действия (например, `chat_invite_user`, `coins_transfer` и тд).
* `action_mid` (int64, опционально) — ID объекта/пользователя действия.
* `action_text` (string, опционально) — текстовое содержание действия
* `message` (string, опционально) — текст сообщения выводимый как обычное сообщение если обработка конкретного экшна не сделана.
* `random_id` (int64, опционально) — случайный ID сообщения.
